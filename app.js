...
//多资源托管  排名分先后
app.use(express.static(path.join(__dirname, 'public','template')));/* 用户端的数据去template去找 */
app.use('/admin',express.static(path.join(__dirname, 'public','admin')));//管理端的数据去admin去找，admin 是别名，决定了访问时 3001/admin
app.use(express.static(path.join(__dirname, 'public')));/* 上面连个都找不到就去public去找 */


//接口响应-用户端 
// 网址中含有的哪个页面  指向那个对应的路由
