...
router.post('/', (req, res, next) => {
  //1. 获取 username、password、nikename、icon、用户名/密码/昵称/头像
  ...
  nikename = nikename || '就你牛x';// 不设置昵称,默认为:就你牛x   (npm 下载一些自动生成昵称的包)
  let follow = 0;  //关注数
  let fans = 0;//粉丝数据
  ...
  if (req.files && req.files.length > 0) {
    ...
    //改名
    fs.renameSync(
      req.files[0].path,//老文件磁盘路径
      req.files[0].path + pathLib.parse(req.files[0].originalname).ext
    )									
	// req.files[0].originalname  老文件原名
	// pathLib.parse(req.files[0].originalname)  老文件原名转成对象   pathLib 在第4行
	// pathLib.parse(req.files[0].originalname).ext  老文件原名转成对象后取出后缀
	 // req.files[0].path + pathLib.parse(req.files[0].originalname).ext  老文件磁盘路径+后缀(查笔记)
          ...
          if (result.length === 0) {
            ...
            collection.insertOne({
              ...
              if (!err) {
                ...
                //插入后的信息，返回给客户端，去掉username,password.不然不安全
                delete result.ops[0].username;
                delete result.ops[0].password;

                res.send({
                  err: 0, msg: '登录页面.注册成功',
                  data: result.ops[0]  //ops刚刚插进去的数据,是个数组 ops[0]刚刚插进去的那一条
                })
              } else {//入库失败
                res.send({ err: 1, msg: '注册页面.注册失败' })
              }
              client.close()
            })
          } else {
